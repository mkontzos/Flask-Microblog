...
class User(PaginatedAPIMixin, UserMixin, AnonymousUserMixin, db.Model):
    ...
    def asked_questions(self):
        questions = Question.query.filter_by(user_id=self.id)
...
class Category(PaginatedAPIMixin, UserMixin, db.Model):
    id = db.Column(db.Integer, primary_key=True)
...
class Answer(PaginatedAPIMixin, UserMixin ,db.Model):
    id = db.Column(db.Integer, primary_key=True)
    body = db.Column(db.String(140))
    timestamp = db.Column(db.DateTime, index=True, default=datetime.utcnow)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'))
